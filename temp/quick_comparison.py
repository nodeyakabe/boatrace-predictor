...
def main():
    ...
    # 改善版: 直接SQLで全データを取得（JOINを最適化）
    print('【Step 2】改善版予測を実行')
    print('  訓練/テストデータをSQLで直接取得...', flush=True)
    start = time.time()
    ...
    print(f'    訓練データ: {len(df_train)}行, {time.time()-start:.1f}秒')
    ...
    print(f'    テストデータ: {len(df_test)}行, {time.time()-start:.1f}秒', flush=True)

    # 選手統計（全期間）
    print('  選手統計を取得...', flush=True)
    start = time.time()
    racer_stats = pd.read_sql_query('''
    SELECT
        e.racer_number,
    ...
    # コース別勝率（最重要）
    print('  コース別勝率を取得...', flush=True)
    start = time.time()
    course_stats = pd.read_sql_query('''
    SELECT
    ...
    # 会場×コース勝率（Opus分析で発見）
    print('  会場×コース勝率を取得...', flush=True)
    start = time.time()
    venue_course = pd.read_sql_query('''
    SELECT
    ...
    # モーター成績
    print('  モーター成績を取得...', flush=True)
    start = time.time()
    motor_stats = pd.read_sql_query('''
    SELECT
