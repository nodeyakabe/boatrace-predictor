...
# 会場別の追い風方向（TW/HWグループ比較方式で検証済み）
# 2026-03-30 全16方位候補を評価、±45度=TWグループ/±135~180度=HWグループのC1勝率差が最大かつ正方向の方位を採用
# 風速2m以上のデータのみ使用。有意差なし(ns)で逆転している定義は削除。
# 検証スクリプト: temp_wind_verify2.py
VENUE_TAILWIND_DIRECTIONS: Dict[str, str] = {
    '01': '南東',    # 桐生    TW=8.8% HW=6.9% diff=+1.9pt z=3.96 p<0.01 (一致)
    '02': '北東',    # 戸田    TW=7.4% HW=7.2% diff=+0.2pt (許容・推奨=北北西だが隣接方位で正方向)
    # '03': 未定義,  # 江戸川   最大diff=+0.5pt ns → 未定義維持
    '04': '東南東',  # 平和島   TW=7.5% HW=6.7% diff=+0.8pt (許容・推奨=南東で隣接)
    '05': '南南西',  # 多摩川   TW=8.8% HW=8.1% diff=+0.7pt (許容・推奨=南東だが正方向)
    # '06': 削除,    # 浜名湖   旧定義=西南西で逆転(diff=-0.3pt)、推奨=南南西もp<0.10で根拠不十分
    '07': '東北東',  # 蒲郡    TW=9.3% HW=8.9% diff=+0.4pt (許容・推奨=東で隣接)
    '08': '南南西',  # 常滑    TW=9.4% HW=9.1% diff=+0.3pt (許容・推奨=東だが正方向)
    '09': '北北東',  # 津     TW=9.8% HW=9.5% diff=+0.3pt (許容・推奨=北北西だが正方向)
    '10': '北北東',  # 三国    TW=9.4% HW=8.8% diff=+0.6pt ns (一致)
    # '11': 削除,    # 琵琶湖   旧定義=北北東で逆転(diff=-0.2pt)、推奨=南西もns(z=0.91)で根拠不十分
    '12': '南東',    # 住之江   修正(旧=北北東 逆転)。TW=9.8%(n=23804) HW=8.8%(n=11430) diff=+1.0pt z=2.99 p<0.01
    '13': '南西',    # 尼崎    TW=10.1% HW=9.7% diff=+0.4pt (許容・推奨=南南西で隣接)
    # '14': 削除,    # 鳴門    旧定義=南南東で逆転(diff=-0.4pt)、推奨=南もns(z=0.65)で根拠不十分
    '15': '東南東',  # 丸亀    TW=9.4% HW=8.8% diff=+0.6pt (許容・推奨=南だが正方向)
    '16': '西南西',  # 児島    TW=9.8% HW=9.2% diff=+0.6pt z=2.24 p<0.05 (一致)
    '17': '西南西',  # 宮島    修正(旧=北北東 逆転)。TW=9.8%(n=33580) HW=9.1%(n=13431) diff=+0.7pt z=2.43 p<0.05
    '18': '南南東',  # 徳山    TW=10.8% HW=9.9% diff=+0.9pt (許容・推奨=南東で隣接)
    # '19': 削除,    # 下関    旧定義=南南西で逆転(diff=-0.2pt)、推奨=東もp<0.10で根拠不十分
    '20': '北北東',  # 若松    TW=9.4% HW=8.7% diff=+0.8pt ns (一致)
    '21': '西南西',  # 芦屋    TW=10.4% HW=10.0% diff=+0.4pt (許容・推奨=南西で隣接)
    '22': '南西',    # 福岡    TW=9.3% HW=8.4% diff=+0.9pt (許容・推奨=西南西で隣接)
    '23': '西北西',  # 唐津    修正(旧=北北東 逆転)。TW=9.6%(n=25487) HW=8.7%(n=39898) diff=+1.0pt z=4.19 p<0.01
    '24': '西南西',  # 大村    TW=10.6% HW=9.3% diff=+1.3pt (許容・推奨=西で隣接)
}
