"""
rdmdb_tideの時系列データから潮位イベント(満潮/干潮)を抽出してtideテーブルに追加
追加対象: 丸亀(15), 児島(16), 若松(20)

観測地点→会場マッピング:
- Hiroshima → 丸亀(15), 児島(16) ※宮島(17)と同じデータ
- Hakata → 若松(20) ※福岡(22)と同じデータ
"""
import sys
import io
...
print("="*80)
print("潮位イベント抽出スクリプト - 追加3会場")
print("="*80)

# 観測地点→会場コードマッピング (追加する3会場のみ)
STATION_TO_VENUES = {
    'Hiroshima': ['15', '16'],  # 丸亀, 児島
    'Hakata': ['20']            # 若松
}
...
for station_name, venue_codes in STATION_TO_VENUES.items():
    for venue_code in venue_codes:
        ...
        with tqdm(total=(max_date - min_date).days + 1, desc=f"会場{venue_code} 処理中") as pbar:
