...
**「異常チェックして」と言われたら【必須手順】**

### STEP 1: チェックスクリプトを実行

```bash
python scripts/monitoring/anomaly_check.py
```

### STEP 2: 結果を以下の形式で口頭報告
...
|------------|:---:|------|
| パイプライン稼働 | ✅/⚠️ | 最終ログ〇〇日前 |
| データ収集 | ✅/⚠️ | before予測カバー率〇〇% |
| A率（信頼度分布） | ✅/⚠️ | advance〇〇% / before〇〇% |
| 直近の実績 | ✅/⚠️ | 直近30日：〇〇件通知 / 〇〇的中 |
| **購入予測品質** | ✅/⚠️ | 展示込み〇〇件 / 直近〇〇件連続OK / 旧レコード〇〇件 |
...
| 指標 | 正常範囲 | 異常の可能性 |
|------|---------|------------|
| advance A率 | 2〜6% | >10% → 旧コード混入（2026-03以降の正規化済み値） |
| before A率 | 15〜27% | <15% または >27% → 異常（実績は2018年から常に20〜26%） |
| before予測カバー率 | ≥90% | <90% → データ収集欠損 |
| schedulerログ | 2日以内 | >2日 → パイプライン停止の可能性 |
| 1日あたり通知件数 | 2〜15件 | 0件 → 自動化停止 |
| 展示なし購入（had_exhibition=0） | 0件 | ≥1件 → monitor force=True修正が未機能 |
| advanceフォールバック購入 | 0件 | ≥1件 → before予測未生成のまま購入（異常） |
| 旧レコード（had_exhibition=NULL） | 時間とともに減少 | 増加 → 新バージョンが使われていない |

### 経過観察ポイント（2026-06-26 force=True修正以降）

- **連続OKストリーク**: `直近N件連続OK` の数値が毎回増えていることを確認
- **旧レコード消滅**: `旧レコード〇〇件(修正前・時間経過で自然消滅)` が30日以内に0になること
- **修正効果開始日**: 初回レポート時に `修正効果開始日: 2026-06-2X` が表示されること
...
```bash
python temp/monthly_stats_fast.py  # 1〜6月比較（時間がかかる）
```
...
### パターン5: 「全会場一括統計 → 会場共通の因果関係」の誤り（2026-03-23 確認）

**誤った推論**:
> 全会場で集計すると「南風時に1コース勝率が高い」→ 南風=追い風=全会場で1コース有利

**なぜ間違いか**:
24競艇場はコースレイアウトが全て異なり、「南風」が追い風になる会場と向かい風になる会場が混在する。全会場を一括集計するとこれらが相殺・歪曲される。分析レポート自体に「会場依存」と正しく注記されていたが、実装時に無視された。

**正しい確認方法**:
- 会場別に個別分析し、各会場で方向の意味を確認する
- 「分析レポート自身が指摘している注意点・前提条件」を実装時に必ず反映させる
- 「地理的・物理的に異なる場所」を一律定義で扱うときは必ず「本当に全会場共通か?」を問う
...
6. **会場・コース別のデータは必ず個別分析** → 24会場は物理的配置が全て異なる。「全会場共通の統計」から会場固有の因果関係を断定しない
