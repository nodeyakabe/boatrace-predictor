...
# -*- coding: utf-8 -*-
"""
2020-2023年全レースの予測生成（並列版）

直前情報がないため、advance予測のみ生成
"""
import sys
import os
import io
...
def save_predictions_batch(predictions_list, db_path):
    ...
    2020-2023年は直前情報がないため、advance予測のみ
    """
...
def main():
    print("=" * 80)
    print("2020-2023年全レース予測生成（並列版・advance予測のみ）")
    print("=" * 80)
    print()
    ...
    print(f"対象期間: 2020-01-01 ～ 2023-12-31")
    print(f"対象日数: {len(dates)}日間")
    print(f"並列ワーカー: {workers}")
    print(f"予測タイプ: advance（直前情報なし）")
    print()
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[{completed:4}/{len(dates)}] {result['date']}: {result['total_races']:3}R "
                      f"(成功:{result['succeeded']:3} 失敗:{result['failed']:2}) "
                      f"[{elapsed/60:.1f}分経過, {races_per_min:.1f}R/分, 残り{eta_min:.1f}分]", flush=True)

            except Exception as e:
                elapsed = time.time() - start_time
                print(f"[{completed:4}/{len(dates)}] {date}: エラー - {str(e)[:30]} [{elapsed/60:.1f}分経過]", flush=True)
