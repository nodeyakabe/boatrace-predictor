...
def main():
    ...
    with tab4:
        ...
        # サブタブで機能を分割
        subtab1, subtab2 = st.tabs(["📦 一括データ収集", "🎯 オリジナル展示収集"])
    ...
    with tab7:
        ...
        st.markdown("---")
        st.subheader("🧪 バックテスト")
        st.markdown("過去データで予想精度を検証")

        col1, col2 = st.columns(2)
        with col1:
            backtest_start = st.date_input("開始日", datetime.now() - timedelta(days=30), key="backtest_start")
        with col2:
            backtest_end = st.date_input("終了日", datetime.now(), key="backtest_end")
        ...
        try:
            ...
            if st.button("バックテスト実行", type="primary"):
                with st.spinner("バックテスト実行中..."):
                    ...
                    if results and results['total_races'] > 0:
                        st.success("バックテスト完了！")
                        ...
                        with col2:
                            st.metric("1着的中率", f"{results.get('rank1_accuracy', 0)*100:.1f}%")
                        with col3:
                            st.metric("三連単的中率", f"{results.get('3tan_hit_rate', 0)*100:.1f}%")
                        with col4:
                            st.metric("二連単的中率", f"{results.get('2tan_hit_rate', 0)*100:.1f}%")

                        with st.expander("詳細結果"):
                            st.json(results)
                    else:
                        st.warning("該当期間のレースデータが見つかりませんでした")
        except Exception as e:
        ...
        try:
            ...
            if st.button("品質チェック実行"):
                with st.spinner("チェック中..."):
                    report = monitor.get_overall_statistics()
                    st.json(report)
        except Exception as e:
            st.error(f"品質チェックエラー: {e}")
