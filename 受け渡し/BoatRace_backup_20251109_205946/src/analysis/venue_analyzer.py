        ...
        try:
            ...
            query = """
                SELECT
                    ...
                    SUM(CASE WHEN chakujun <= 2 THEN 1 ELSE 0 END) as place2_count,
                    SUM(CASE WHEN chakujun <= 3 THEN 1 ELSE 0 END) as place3_count
                FROM race_results
                WHERE venue_code = ?
                  AND race_date >= ?
                GROUP BY waku
        ...
        try:
            ...
            query = """
                ...
                WHERE venue_code = ?
                  ...
                  AND kimarite != ''
                GROUP BY waku, kimarite
                ORDER BY waku, count DESC
            """
            ...
            for season_name, months in seasons.items():
                ...
                query = f"""
                    SELECT
                        ...
                        COUNT(*) as total_races,
                        SUM(CASE WHEN chakujun = 1 THEN 1 ELSE 0 END) as win_count,
                        CAST(SUM(CASE WHEN chakujun = 1 THEN 1 ELSE 0 END) AS FLOAT) / COUNT(*) * 100 as win_rate
                    FROM race_results
                    WHERE venue_code = ?
                      AND CAST(strftime('%m', race_date) AS INTEGER) IN ({months_str})
                    GROUP BY waku
                    ORDER BY waku
                """
