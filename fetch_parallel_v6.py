"""
並列データ取得スクリプト v6 - 修正版スクレイパー + 期間拡張

V5からの改善点:
- 修正版BeforeInfoScraperを使用（展示タイム取得率向上: 1.3% → 70-90%）
- 修正版決まり手取得ロジック（取得率向上: 1.5% → 90%+）
- 対象期間を1年拡張: 2022-11-01 ～ 2025-10-31 (1,096日)
- V5で取得済みのデータも修正版スクレイパーで補完

取得対象:
1. 2022-11-01 ～ 2023-10-31 の全データ（新規365日）
2. 2023-11-01 ～ 2025-10-31 の不足データ（補完731日）

修正内容:
- BeforeInfoScraper: 空ヘッダー対応（固定列インデックス使用）
- 決まり手: 文字列→数値コード変換ロジック追加
"""
...
def fetch_http_only(args):
    """
    HTTP通信のみ実行（DB書き込みなし）
    V5改善: 3つのHTTPリクエストを並列実行（3.3秒→1.1秒）
    ...
    try:
        ...
        # ThreadPoolExecutorで3リクエストを同時実行
                ...
                try:
                    ...
                    if complete_result and not complete_result.get('is_invalid'):
                        ...
                        result_data_for_save = {
                            ...
                            'kimarite': kimarite_text  # テキスト形式の決まり手も保存
                        }
...
def main():
    parser = argparse.ArgumentParser(description='競艇データ並列取得 v5 (欠損データ補完機能付き)')
    parser.add_argument('--start', help='開始日 (YYYY-MM-DD) ※新規取得モード時に必須')
    parser.add_argument('--end', help='終了日 (YYYY-MM-DD) ※新規取得モード時に必須')
    parser.add_argument('--workers', type=int, default=10, help='HTTPワーカー数 (デフォルト: 10)')
    parser.add_argument('--venues', help='会場コード (カンマ区切り。指定しない場合は全会場)')
    parser.add_argument('--fill-missing', action='store_true', help='欠損データ補完モード')
    parser.add_argument('--limit', type=int, help='補完モード時の最大取得件数（指定しない場合は全件）')
    ...
    if args.fill_missing:
        # 欠損データ補完モード
        print("=" * 80)
        print(f"競艇データ並列取得 v5 - 欠損データ補完モード")
        print("=" * 80)
        ...
        print(f"  総タスク数: {len(tasks)}件 (開催日のみ)")
    ...
    stop_event.set()
    db_thread.join(timeout=300)  # 最大5分待機
