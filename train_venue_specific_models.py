...
print(f"{'会場コード':<12} {'レース数':<10}")
print("-" * 30)
for _, row in df_venue_stats.iterrows():
    print(f"{row['venue_code']:<12} {row['race_count']:<10}")

# 上位5会場を選択
top_venues = df_venue_stats.head(5)['venue_code'].tolist()
print(f"\n対象会場: {top_venues}")
...
for venue_code in top_venues:
    ...
    # テストデータ
    print(f"[Step 3-{venue_code}] テストデータ構築")
    ...
    if len(df_train_raw) < 1000 or len(df_test_raw) < 50:
        print(f"  [スキップ] データ量不足")
        continue
    ...
    print(f"[Step 4-{venue_code}] 学習実行")
...
print(f"\n{'会場':<10} {'学習':<8} {'テスト':<8} {'AUC':<8} {'Log Loss':<10} {'的中率(0.8+)':<15}")
print("-" * 80)
for _, row in df_results.iterrows():
    print(f"{row['venue_code']:<10} {row['train_count']:<8} {row['test_count']:<8} {row['auc']:<8.4f} {row['logloss']:<10.4f} {row['hit_rate_08']:<15.2%}")

print(f"\n平均性能:")
print(f"  AUC: {df_results['auc'].mean():.4f}")
print(f"  Log Loss: {df_results['logloss'].mean():.4f}")
print(f"  的中率(0.8+): {df_results['hit_rate_08'].mean():.2%}")

# 実験#012（全会場統合モデル）との比較
print(f"\n実験#012（全会場統合）との比較:")
print(f"  実験#012 AUC: 0.8496")
print(f"  会場別平均 AUC: {df_results['auc'].mean():.4f}")
print(f"  差分: {df_results['auc'].mean() - 0.8496:+.4f}")
