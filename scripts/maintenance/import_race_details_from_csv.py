# -*- coding: utf-8 -*-
"""CSVファイルからrace_detailsをDBに一括投入

使用例:
    # 検証モード（dry-run）
    python scripts/maintenance/import_race_details_from_csv.py \
        --input data/csv/race_details/2020 \
        --dry-run
...
def insert_race_details(
    ...
    for i, row in enumerate(details_data, 1):
        ...
        if not race_id:
            failed += 1
            if i % 1000 == 0:
                print(f"  [{i}/{len(details_data)}] race_id未発見: {venue_code} {race_date} R{race_number}")
            continue
        ...
        try:
            if existing:
                ...
                """, (
                    ...
                    float(row['chikusen_time']) if row.get('chikusen_time') else None,
                ...
                cursor.execute("""
                    INSERT INTO race_details (
                        race_id, pit_number, exhibition_time, tilt,
                        chikusen_time, st_time
                    ) VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    race_id,
                    pit_number,
            ...
            # 進捗表示
            if i % 1000 == 0:
                print(f"  [{i}/{len(details_data)}] 投入中... (成功: {inserted}, スキップ: {skipped}, 失敗: {failed})")
...
def main():
    parser = argparse.ArgumentParser(description='CSVからrace_detailsをDB投入')
    ...
    csv_file = input_dir / 'race_details.csv'
    ...
    print("="*80)
    if args.dry_run:
        print("race_details DB投入（検証モード）")
    else:
        print("race_details DB投入")
    print("="*80)
    ...
    # CSVロード
    print("CSVファイルを読み込み中...")
    details_data = load_csv(csv_file)
    print(f"  読み込み完了: {len(details_data):,}件")
    print()
    ...
    try:
        ...
        # データ投入
        print("race_detailsを投入中...")
        result = insert_race_details(conn, details_data, args.overwrite, args.dry_run)
