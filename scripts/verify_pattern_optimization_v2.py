...
# -*- coding: utf-8 -*-
"""
直前情報最適化の効果検証スクリプト v2

実際のパターンスコアラーのロジックを再現し、
2024年データで新旧ロジックを比較検証する。
"""
...
def calculate_old_score(pre_rank, ex_rank, st_rank, original_score):
    ...
    if 2 <= pre_rank <= 3:
        # PRE2-3位 & 展示1-2位
        if ex_rank <= 2:
            multiplier = max(multiplier, 1.084)
            applied_patterns.append(('pre2_3_ex1_2', 1.084))
        # 展示1-3位 & PRE2-3位
        if ex_rank <= 3:
            multiplier = max(multiplier, 1.069)
            applied_patterns.append(('ex1_3_pre2_3', 1.069))

    # 展示2位
    if ex_rank == 2:
        multiplier = max(multiplier, 1.035)
        applied_patterns.append(('ex_rank_2', 1.035))

    # TOP3パターン
    # PRE1-4位 & 展示1-2位
    if pre_rank <= 4 and ex_rank <= 2:
        multiplier = max(multiplier, 1.104)
        applied_patterns.append(('pre1_4_ex1_2', 1.104))

    # 展示1-2位
    if ex_rank <= 2:
        multiplier = max(multiplier, 1.051)
        applied_patterns.append(('ex_rank_1_2', 1.051))
...
def run_backtest():
    ...
    for race_id, venue_code, race_date in races:
        ...
        for pred in predictions:
            ...
            for pattern_name, mult in new_patterns:
                ...
                # ネガティブパターンの効果チェック
                if pattern_name in negative_pattern_effect:
                    if pre_rank == 1:  # PRE1位だった艇に適用された場合
                        negative_pattern_effect[pattern_name]['blocked'] += 1
                        if pit == actual_first:
                            negative_pattern_effect[pattern_name]['would_have_hit'] += 1
    ...
    print(f"差分（新-旧）: {new_rate - old_rate:+.2f}pt")
    print(f"差分（新-ベースライン）: {new_rate - baseline_rate:+.2f}pt")
    print()

    # === パターン別効果 ===

    print("=" * 80)
    print("2. パターン別効果（適用頻度順）")
    print("=" * 80)

    sorted_patterns = sorted(pattern_stats.items(), key=lambda x: x[1]['applied'], reverse=True)

    # 新規追加パターンをハイライト
    new_patterns_list = ['pre1_st1_ex1', 'pre1_st6', 'pre1_st5_6', 'pre1_st4_6',
                         'pre2_st1_2', 'pre1_3_st1', 'st1_2_ex1_2_double_top',
    ...
    for name, stats in sorted_patterns:
        ...
        is_new = "***" if name in new_patterns_list else ""
        ...
        pattern_results.append({
            ...
            'is_new': name in new_patterns_list
        })

        print(f"  {name}{is_new}: {correct}/{applied} ({rate:.2f}%) | 平均倍率: {avg_mult:.3f}")
    ...
    print("=" * 80)
    print("3. ネガティブパターンの効果")
    print("=" * 80)

    for name, stats in negative_pattern_effect.items():
        blocked = stats['blocked']
        would_hit = stats['would_have_hit']
        if blocked > 0:
            miss_rate = would_hit / blocked * 100
            print(f"  {name}:")
            print(f"    適用回数: {blocked}")
            print(f"    そのうち実際に1着だった: {would_hit} ({miss_rate:.1f}%)")
            print(f"    → 正しくブロックした: {blocked - would_hit} ({100 - miss_rate:.1f}%)")
        else:
            print(f"  {name}: 適用なし")
    ...
    for conf in ['B', 'C', 'D']:
        ...
        if new_stats['total'] > 0:
            ...
            print(f"    新ロジック: {new_rate:.2f}%")
            print(f"    旧ロジック: {old_rate:.2f}%")
    ...
    for v in venue_results[:10]:
        print(f"  会場{v['venue']}: 新={v['new_rate']:.1f}% | 旧={v['old_rate']:.1f}% | 差分={v['diff']:+.1f}pt (n={v['new_total']})")
    ...
    if se > 0:
        ...
        print(f"  サンプルサイズ: 新={n1}, 旧={n2}")
        print(f"  1着的中率: 新={p1*100:.2f}%, 旧={p2*100:.2f}%")
    ...
    if diff_vs_old > 0.5:
        recommendation = "継続"
        reason = "新ロジックが旧ロジックを上回っており、統計的にも実用的にも効果が認められる"
    elif diff_vs_old > -0.5:
        recommendation = "調整"
        reason = "新旧ロジックの差は微小であり、パターンの調整を検討すべき"
    else:
        recommendation = "要再検討"
        reason = "新ロジックが旧ロジックを下回っており、実装の見直しが必要"
    ...
    # ポジティブパターン
    positive_patterns = ['pre1_st1_ex1', 'pre1_st1', 'pre2_st1_2', 'pre1_3_st1', 'st1_2_ex1_2_double_top']
    for p in positive_patterns:
        if p in pattern_stats:
            stats = pattern_stats[p]
            rate = stats['correct'] / stats['applied'] * 100 if stats['applied'] > 0 else 0
            if rate > 50:
                verdict = "効果的"
            elif rate > 30:
                verdict = "要調整"
    ...
    for name, stats in negative_pattern_effect.items():
        if stats['blocked'] > 0:
            miss_rate = stats['would_have_hit'] / stats['blocked'] * 100
            if miss_rate < 30:
                verdict = "効果的（正しくブロック）"
            elif miss_rate < 50:
                verdict = "やや効果的"
                ...
                verdict = "要調整（過剰ブロック）"
            print(f"    {name}: 誤ブロック率{miss_rate:.1f}% → {verdict}")
    ...
    # パターン別効果CSV
    pattern_csv = output_dir / 'pattern_effects_v2.csv'
    with open(pattern_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['pattern', 'applied', 'correct', 'rate', 'avg_multiplier', 'is_new'])
        writer.writeheader()
        writer.writerows(pattern_results)
    print(f"パターン別効果: {pattern_csv}")

    # 信頼度別効果CSV
    confidence_csv = output_dir / 'confidence_effects_v2.csv'
    ...
    # 会場別効果CSV
    venue_csv = output_dir / 'venue_effects_v2.csv'
