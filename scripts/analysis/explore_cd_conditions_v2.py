...
# -*- coding: utf-8 -*-
"""
信頼度C,D向け新規購入条件探索スクリプト v2

目的:
    信頼度C,D向けの新規購入条件候補を発見する（詳細版）
...
使用方法:
    python scripts/analysis/explore_cd_conditions_v2.py
...
EXISTING_CONDITIONS = [
    # C条件
    {'confidence': 'C', 'c1_ranks': ['B1'], 'odds_min': 20, 'odds_max': 30,
     'venue_filter': [23, 18, 5, 4, 9, 15, 8, 24, 20, 17], 'name': 'C×20-30×B1+会場'},
    {'confidence': 'C', 'c1_ranks': ['A2'], 'odds_min': 30, 'odds_max': 80,
     'venue_filter': [14], 'name': '鳴門×C×A2×30-80'},
    # D条件
    {'confidence': 'D', 'c1_ranks': ['B1'], 'odds_min': 40, 'odds_max': 50,
     'c1_second_rate_min': 20, 'c1_second_rate_max': 30, 'name': 'D×40-50×B1×2連率20-30%'},
    {'confidence': 'D', 'predicted_course': 5, 'name': 'D×5コース予測'},
]
...
ODDS_RANGES = [
    ...
    (40, 50), (45, 55), (50, 60), (60, 80), (80, 100), (100, 150),
]
...
def analyze_condition(cursor, **kwargs) -> Dict:
    """条件をバックテスト"""
    results = {
        'yearly': {},
        'total_bets': 0,
    ...
    query = build_exploration_query(
        ...
        c1_ranks=kwargs['c1_ranks'],
        odds_min=kwargs['odds_min'],
        odds_max=kwargs['odds_max'],
        date_start='2020-01-01',
        date_end='2026-01-01',
        venue_code=kwargs.get('venue_code'),
        predicted_course=kwargs.get('predicted_course'),
        motor_min=kwargs.get('motor_min'),
        c1_second_rate_min=kwargs.get('c1_second_rate_min'),
        c1_second_rate_max=kwargs.get('c1_second_rate_max'),
    )
...
def check_adoption_criteria(result: Dict) -> Tuple[bool, str]:
    ...
    black_count = sum(1 for year in recent_years
...
def is_overlapping_with_existing(condition: Dict) -> bool:
    """既存条件と重複するかチェック"""
    for existing in EXISTING_CONDITIONS:
        ...
        # 予測コース条件の場合
        if 'predicted_course' in existing:
            if condition.get('predicted_course') == existing['predicted_course']:
                return True
            continue

        # 会場フィルターの場合
        if 'venue_filter' in existing:
            if condition.get('venue_code') in existing['venue_filter']:
                # オッズ帯が重複する場合
                if (condition['odds_min'] < existing['odds_max'] and
                    condition['odds_max'] > existing['odds_min']):
                    # 級別が重複する場合
                    if any(rank in existing['c1_ranks'] for rank in condition['c1_ranks']):
                        return True
            continue

        # 2連対率条件の場合
        if 'c1_second_rate_min' in existing:
            if (condition.get('c1_second_rate_min') == existing['c1_second_rate_min'] and
                condition.get('c1_second_rate_max') == existing['c1_second_rate_max']):
                if (condition['odds_min'] < existing['odds_max'] and
                    condition['odds_max'] > existing['odds_min']):
                    if any(rank in existing['c1_ranks'] for rank in condition['c1_ranks']):
                        return True
...
def explore_conditions():
    ...
    # Phase 1: D条件の詳細探索（基本）
    # ============================================================
    print("Phase 1: D条件 基本探索（級別×オッズ帯）")
    print("-" * 80)
    ...
    for c1_rank in C1_RANKS:
        for odds_min, odds_max in ODDS_RANGES:
            ...
            result = analyze_condition(cursor, **condition)

            if result['total_bets'] == 0:
                continue

            condition_name = format_condition_name('D', [c1_rank], odds_min, odds_max)
            passed, reason = check_adoption_criteria(result)
            is_overlapping = is_overlapping_with_existing(condition)

            condition_data = {**condition, 'name': condition_name, 'result': result,
                             'passed': passed, 'reason': reason, 'overlapping': is_overlapping}
            all_conditions.append(condition_data)

            if passed and not is_overlapping:
                adopted_conditions.append(condition_data)
                print(f"[NEW] {condition_name}")
                print(f"      件数: {result['total_bets']}, ROI: {result['total_roi']:.1f}%, 収支: {result['total_profit']:+,.0f}円")

    # ============================================================
    # Phase 2: D条件 予測コース探索（全オッズ帯）
    # ============================================================
    print()
    print("Phase 2: D条件 予測コース探索")
    print("-" * 80)

    for predicted_course in PREDICTED_COURSES:
        # 広いオッズ帯での検証
        for odds_min, odds_max in [(10, 50), (10, 100), (10, 200), (30, 100), (50, 150)]:
            condition = {
                'confidence': 'D',
                'c1_ranks': C1_RANKS,
                'odds_min': odds_min,
            ...
            condition_name = format_condition_name(
                'D', C1_RANKS, odds_min, odds_max, predicted_course=predicted_course
            )
            passed, reason = check_adoption_criteria(result)
            is_overlapping = is_overlapping_with_existing(condition)
    ...
    for c1_rank in C1_RANKS:
        for rate_min, rate_max in second_rate_ranges:
            for odds_min, odds_max in ODDS_RANGES:
                condition = {
                    'confidence': 'D',
                    'c1_ranks': [c1_rank],
                ...
                condition_name = format_condition_name(
                    'D', [c1_rank], odds_min, odds_max,
                    c1_second_rate_min=rate_min, c1_second_rate_max=rate_max
                )
                passed, reason = check_adoption_criteria(result)
                is_overlapping = is_overlapping_with_existing(condition)
    ...
    # ============================================================
    print()
    print("Phase 4: D条件 会場別探索")
    print("-" * 80)

    for venue_code in range(1, 25):
        for c1_rank in C1_RANKS:
            for odds_min, odds_max in [(30, 50), (40, 60), (50, 80), (60, 100), (80, 120)]:
                condition = {
                    'confidence': 'D',
                    'c1_ranks': [c1_rank],
                    'odds_min': odds_min,
                ...
                condition_name = format_condition_name(
                    'D', [c1_rank], odds_min, odds_max, venue_code=venue_code
                )
                passed, reason = check_adoption_criteria(result)
                is_overlapping = is_overlapping_with_existing(condition)
    ...
    # ============================================================
    # Phase 5: C条件 新規探索（既存と重複しない）
    # ============================================================
    print()
    print("Phase 5: C条件 新規探索")
    print("-" * 80)
    ...
    for venue_code in range(1, 25):
        for c1_rank in C1_RANKS:
            for odds_min, odds_max in ODDS_RANGES:
                ...
                condition_name = format_condition_name(
                    'C', [c1_rank], odds_min, odds_max, venue_code=venue_code
                )
                passed, reason = check_adoption_criteria(result)
                is_overlapping = is_overlapping_with_existing(condition)
    ...
    print()
    print("=" * 100)
    print("新規採用候補一覧（既存条件と重複しない）")
    print("=" * 100)

    # ROI順にソート
    adopted_conditions.sort(key=lambda x: x['result']['total_roi'], reverse=True)

    print(f"{'No':<4} {'条件名':<45} {'件数':>8} {'ROI':>8} {'収支':>14} {'直近4年黒字'}")
    print("-" * 105)
    ...
    for i, cond in enumerate(adopted_conditions, 1):
        ...
        recent_years = [2022, 2023, 2024, 2025]
        black_years = [str(year)[-2:] for year in recent_years
                       if year in result['yearly'] and result['yearly'][year]['profit'] > 0]
        black_str = ','.join(black_years) if black_years else '-'

        print(f"{i:<4} {cond['name']:<45} {result['total_bets']:>8} {result['total_roi']:>7.1f}% {result['total_profit']:>+14,.0f} {len(black_years)}/4年({black_str})")

    print("-" * 105)
    print(f"合計: {len(adopted_conditions)}件の新規候補")
    ...
    print()
    print("=" * 100)
    print("上位候補の年度別詳細（ROI上位30件）")
    print("=" * 100)

    for i, cond in enumerate(adopted_conditions[:30], 1):
        result = cond['result']
        print(f"\n{i}. {cond['name']}")
