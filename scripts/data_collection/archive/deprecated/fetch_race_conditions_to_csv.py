# -*- coding: utf-8 -*-
"""race_conditionsデータをCSVに収集（wave_height補完用）

DB負荷を回避するため、CSVに収集してから後でバルク投入する。

使用例:
    # 2020-2024年のrace_conditionsを収集
...
def fetch_race_conditions(venue_code: str, race_date: str, race_number: int) -> Optional[Dict]:
    """
    1レース分のrace_conditionsを取得
...
def fetch_venue_day(args) -> Dict:
    ...
    for race_number in range(1, 13):  # 1R-12R
        try:
            conditions = fetch_race_conditions(venue_code, race_date, race_number)
            if conditions:
...
def save_to_csv_batch(output_file: Path, data: List[Dict], is_first_batch: bool = False):
    ...
    header = ['venue_code', 'race_date', 'race_number', 'weather', 'wind_direction',
...
def main():
    parser = argparse.ArgumentParser(description='race_conditionsデータをCSVに収集')
    ...
    output_file = output_dir / 'race_conditions.csv'

    print("="*80)
    print("race_conditions CSV収集（wave_height補完用）")
    print("="*80)
    ...
    print()
    print("次のステップ:")
    print(f"  python scripts/maintenance/import_race_conditions_from_csv.py --input {output_dir}")
    print("="*80)
