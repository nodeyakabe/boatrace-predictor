# -*- coding: utf-8 -*-
"""2021年・2023年の欠損データ補完スクリプト（完全版）

修正内容:
1. ✅ オッズデータ保存バグ修正（odds_data.get('odds') → odds_data）
2. ✅ 展示情報収集機能追加（BeforeInfoScraper）
...
制限事項:
- 展示情報は前日のみ公開されるため、過去レースでは取得できない場合があります
- 過去レースでは空のデータとなる可能性が高いですが、試行する価値はあります
"""
...
from src.scraper.beforeinfo_scraper import BeforeInfoScraper  # ← 追加
...
def get_scrapers():
    ...
    if not hasattr(thread_local, 'race_scraper'):
        ...
        thread_local.beforeinfo_scraper = BeforeInfoScraper()  # ← 追加
    return (thread_local.race_scraper, thread_local.result_scraper,
            thread_local.odds_scraper, thread_local.beforeinfo_scraper)  # ← 追加
...
def check_missing_data(db_path: Path, year: int, month: int = None):
    ...
    cursor.execute('''
        ...
        WHERE r.race_date LIKE ? AND rd.race_id IS NULL
    ''', (date_pattern,))
    ...
    return {
        ...
        'missing_details': missing_details  # ← 追加
    }
...
def fetch_venue_day_parallel(args):
    ...
    race_scraper, result_scraper, odds_scraper, beforeinfo_scraper = get_scrapers()  # ← 修正

    race_date_yyyymmdd = race_date.replace('-', '')
    success_count = 0
    races_data = []
    incomplete_results = []
    beforeinfo_success = 0  # ← 追加
    beforeinfo_failed = 0   # ← 追加
    ...
    for race_number in range(1, 13):
        ...
        for retry in range(max_retries):
            try:
                ...
                try:
                    beforeinfo_data = beforeinfo_scraper.get_race_beforeinfo(venue_code, race_date_yyyymmdd, race_number)
                    if beforeinfo_data and beforeinfo_data.get('exhibition_times'):
                        beforeinfo_success += 1
                        # entriesに展示情報をマージ
                        for entry in race_data.get('entries', []):
                            pit = entry.get('pit_number')
                            if pit:
                                entry['exhibition_time'] = beforeinfo_data.get('exhibition_times', {}).get(pit)
                                entry['tilt_angle'] = beforeinfo_data.get('tilt_angles', {}).get(pit)
                                entry['parts_replacement'] = beforeinfo_data.get('parts_replacements', {}).get(pit, '')
                                entry['st_time'] = beforeinfo_data.get('start_timings', {}).get(pit)
                                entry['actual_course'] = beforeinfo_data.get('exhibition_courses', {}).get(pit)
                                entry['adjusted_weight'] = beforeinfo_data.get('adjusted_weights', {}).get(pit)
                    else:
                        beforeinfo_failed += 1
                except Exception as e:
                    beforeinfo_failed += 1
                    # 展示情報が取得できなくてもスキップ（過去レースでは取得不可能な場合が多い）
                    pass
                ...
                races_data.append({
                    ...
                    'result': result_data,
                    'odds': odds_data,
                    'beforeinfo': beforeinfo_data  # ← 追加（ログ用）
                })
                success_count += 1
    ...
    # 展示情報の取得状況をログ出力
    if beforeinfo_success > 0:
        print(f"[展示情報] {venue_code} {race_date}: {beforeinfo_success}件成功, {beforeinfo_failed}件失敗")
...
def save_to_csv(output_dir: Path, all_races_data: list):
    ...
    details_header = ['venue_code', 'race_date', 'race_number', 'pit_number', 'exhibition_time',
                      'tilt_angle', 'parts_replacement', 'actual_course', 'st_time', 'adjusted_weight']  # ← 修正
        ...
        for item in all_races_data:
            ...
            for race_item in races_data:
                try:
                    ...
                    for entry in race_data.get('entries', []):
                        # 展示情報がある場合のみ保存
                        if (entry.get('exhibition_time') is not None or
                            entry.get('actual_course') is not None or
                            entry.get('st_time') is not None):
                            detail_row = {
                                'venue_code': venue_code,
                                'race_date': race_date,
                                'race_number': race_data.get('race_number'),
                                ...
                                'adjusted_weight': entry.get('adjusted_weight'),  # ← 追加
                            }
...
def main():
    parser = argparse.ArgumentParser(description='2021年・2023年欠損データ補完（完全版）')
    ...
    parser.add_argument('--workers', type=int, default=8, help='並列数（デフォルト: 8）※展示情報収集のため控えめ')
    ...
    print("=" * 80)
    print(f"{args.year}年欠損データ補完スクリプト（完全版）")
    print("=" * 80)
    print()
    print("【修正内容】")
    print("✅ オッズデータ保存バグ修正")
    print("✅ 展示情報収集機能追加（BeforeInfoScraper）")
    print()
    ...
    print(f"  展示情報欠損: {year_stats['missing_details']:,} ({year_stats['missing_details']/year_stats['total_races']*100:.1f}%)")
    print()
    ...
    for month in range(1, 13):
        ...
        if month_stats['total_races'] > 0:
            print(f"{month:2d}月: 総{month_stats['total_races']:4d}レース | "
                  ...
                  f"展示欠{month_stats['missing_details']:4d}")
    print()
    ...
    for month in target_months:
        ...
        print(f"予想時間: 約{len(dates) * 0.7 / 60:.1f}-{len(dates) * 1.5 / 60:.1f}時間（展示情報収集含む）")
        print()
    ...
    print("=" * 80)
    print("\n次のステップ:")
    print(f"1. データ投入: python scripts/maintenance/投入_2021_2023_補完データ.py --year {args.year} --all-months")
    print()
