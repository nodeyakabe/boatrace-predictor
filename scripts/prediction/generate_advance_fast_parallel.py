...
# -*- coding: utf-8 -*-
"""
advance予測を並列生成するスクリプト（CSV方式対応版）

generate_before_fast_parallel.py の advance版。
generate_advance_fast.py のN並列版。
...
【before版との違い】
  - use_beforeinfo=False（展示タイム不使用）
  - exhibition_timeフィルターなし（全レースが対象）
  - _apply_environmental_penalty なし（advance予測には不要）
  - env_cacheプリロード不要

【使用方法】
  # 6並列（デフォルト）
  python scripts/prediction/generate_advance_fast_parallel.py --year 2025 --force

  # ワーカー数指定
  python scripts/prediction/generate_advance_fast_parallel.py --year 2025 --force --workers 4

  # CSVのみ（DB投入なし）
  python scripts/prediction/generate_advance_fast_parallel.py --year 2025 --force --csv-only

  # DB投入のみ（既存CSVから）
  python scripts/prediction/generate_advance_fast_parallel.py --year 2025 --import-only
...
【注意】
  ...
  - CSVは data/predictions_csv/advance/YYYY/YYYY-MM-DD.csv に保存
"""
import sys
import os
import io
...
def get_remaining_races(year, start_date=None, end_date=None, force=False):
    """advance未生成のレースを一括取得（entriesが存在するもの）"""
    ...
    return remaining, total_races, already_done
...
def worker_generate(args_tuple):
    """
    ワーカープロセス: 担当日付範囲のレースをCSVに書き出す（advance版）
    ...
    for date_idx, race_date in enumerate(date_list):
        ...
        if use_batch:
            # バッチ処理: 1日分を一括推論（use_beforeinfo=False）
            try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='advance予測を並列生成（N並列でCSV方式）'
    )
    ...
    print("=" * 70)
    print(f"{year}年 advance予測 並列生成（{args.workers}並列・CSV方式）  [{range_str}]")
    if args.import_only:
    ...
    print(f"全レース数（entries有）: {total_races:,}件")
    ...
    print(f"  ※ 各ワーカーが独立した Predictor + BatchDataLoader を初期化（メモリ: 約{n_workers}×500MB~1GB）")
    ...
    if args.csv_only:
        print(f"\n--csv-only: DB投入はスキップ。")
        print(f"後で投入する場合:")
        print(f"  python scripts/prediction/generate_advance_fast_parallel.py --year {year} --import-only")
        return
