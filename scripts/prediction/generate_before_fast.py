...
【CSVファイル配置】
  data/predictions_csv/before/{YYYY}/{YYYY-MM-DD}.csv  （1日1ファイル）

【その他】
- exhibition_time ありのレースのみ対象（直前情報が存在するレース）
- 未生成分のみ処理（既存DBのbefore予測が存在する日付はスキップ）
- コマンドライン引数で年度・期間指定
- EnvironmentalPenaltySystem は CSV書き出し時（Phase 1）に適用済み
"""
import sys
import os
import io
...
def get_csv_path(race_date: str) -> str:
    """日付からCSVパスを生成: data/predictions_csv/before/YYYY/YYYY-MM-DD.csv"""
    year = race_date[:4]
...
def get_remaining_races(year, start_date=None, end_date=None, force=False):
    ...
    # 対象範囲の全レースを取得（entriesが存在するもののみ、日付順）
    # entries欠損レースはpredict_race()が[]を返すためスキップして効率化する
    cursor.execute('''
...
def phase1_generate_to_csv(remaining_races, predictor) -> dict:
    ...
    EnvironmentalPenaltySystem は DataManager._apply_environmental_penalty() 経由で適用。
    ...
    for date_idx, (race_date, day_races) in enumerate(grouped.items()):
        ...
        if use_batch:
            ...
            for race_id, race_date_r, venue_code, race_number in day_races:
                ...
                if predictions:
                    # before の場合、EnvironmentalPenalty を適用
...
def main():
    ...
    parser = argparse.ArgumentParser(description='before予測を高速生成（CSV方式）')
    ...
    print("=" * 70)
    print(f"{year}年 before予測 高速生成（CSV方式）  [{range_str}]")
    if args.csv_only:
    ...
    remaining_races, total_with_beforeinfo, already_done = get_remaining_races(year, start_date, end_date, force=args.force)
    ...
    if args.csv_only:
        ...
        print(f"  python scripts/prediction/generate_before_fast.py --import-only {year_dir}")
        return
