...
# -*- coding: utf-8 -*-
"""
2022-2024年の事前予想（advance）を再生成するスクリプト（高速版）
- 現在のフィーチャーフラグ設定で予測を再生成
- applied_rulesに適用ルールを記録
"""
import sys
import os
import io
...
def get_races_to_regenerate(start_year='2022', end_year='2024'):
    """再生成対象のレースを取得（既存予測を削除して再生成）"""
    ...
    cursor.execute(f'''
        ...
        AND race_id IN (SELECT id FROM races WHERE race_date >= '{start_year}-01-01' AND race_date <= '{end_year}-12-31')
    ''')
...
def main():
    print("=" * 70)
    print("2022-2024年 事前予想（advance）再生成（高速版）")
