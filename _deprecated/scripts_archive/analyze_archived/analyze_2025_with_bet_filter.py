"""
2025年全データでの予想精度分析（BetTargetEvaluator適用版）

現在の運用ロジック（購入対象フィルタリング）を適用した分析
- オッズ範囲フィルタ
- 1コース選手の級別フィルタ
- 信頼度別条件
"""
...
def main():
    print("=" * 80)
    print("2025年全データ予想精度分析（購入対象フィルタ適用版）")
    print("=" * 80)
    print()
    ...
    # BetTargetEvaluatorを初期化
    evaluator = BetTargetEvaluator(str(db_path))
    ...
    # オッズ範囲別統計
    odds_range_stats = defaultdict(lambda: {
        'total': 0,
        'hit': 0,
        'total_invested': 0,
        'total_return': 0
    ...
    for idx, (race_id, race_date, venue_code, race_number) in enumerate(all_races):
        if (idx + 1) % 1000 == 0:
            print(f"処理中: {idx+1:,}/{len(all_races):,}レース... (購入対象: {target_count:,})", flush=True)

        try:
            # 購入対象判定（事前情報）
            bet_target = evaluator.evaluate_race(race_id)

            # 購入対象外の場合はスキップ
            if bet_target is None or bet_target.status == BetStatus.EXCLUDED:
                continue

            target_count += 1

            # race_predictionsから予想上位3艇を取得
            cursor.execute('''
            ...
            # 投資額（bet_targetの推奨額を使用）
            investment = bet_target.bet_amount
            ...
            # オッズ範囲別統計更新
            odds_range_stats[odds_range]['total'] += 1
            if is_hit:
                odds_range_stats[odds_range]['hit'] += 1
            odds_range_stats[odds_range]['total_invested'] += investment
            odds_range_stats[odds_range]['total_return'] += return_amount
    ...
    print(f"\n処理完了")
    print(f"  総レース数: {len(all_races):,}レース")
    print(f"  購入対象: {target_count:,}レース")
    print(f"  分析完了: {processed:,}レース")
    print()

    # 結果表示
    print("=" * 80)
    print("全体評価結果（購入対象レースのみ）")
    print("=" * 80)
    print()

    if overall_stats['total'] > 0:
        print(f"購入対象レース数: {overall_stats['total']:,}レース")
        print()
    ...
    for odds_range in sorted(odds_range_stats.keys()):
        ...
        print(f"{odds_range}: {stats['total']:,}R | 的中率 {hit_rate:.2f}% | ROI {roi:.2f}% | 収支 {profit:+,.0f}円")
