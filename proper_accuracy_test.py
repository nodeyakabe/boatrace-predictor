"""
正しい期間での精度比較テスト
race_details + results 両方あるレース全て(1,285レース)で検証
"""
...
def main():
    ...
    # race_details + results 両方あるレース全て取得
    cursor = conn.cursor()
    cursor.execute("""
    ...
    for i, (race_id, race_date, venue, race_no) in enumerate(test_races):
        if i % 100 == 0:
            print(f"進捗: {i}/{len(test_races)} ({i/len(test_races)*100:.1f}%)")
